...
TS_adeq = c(GRW_adeq_passed, URW_adeq_passed, stasis_adeq_passed, strict_stasis_adeq_passed,
            decel_adeq_passed, accel_adeq_passed, OU_adeq_passed, OU_mov_opt_anc_adeq_passed, OU_mov_opt_adeq_passed)
...
metadata_model_test_adeq <- metadata[metadata$tsID %in% names(TS_adeq), ] 
metadata_model_test_adeq$tsID = as.character(metadata_model_test_adeq$tsID)
model_test_adeq <- model_test[names(model_test) %in% names(TS_adeq)] 

# missing_ids <- setdiff(names(model_test), metadata_model_test_adeq$tsID)
...
model_aicc_min <- lapply(model_test_adeq, function(aicc_min) {
...
histogram_main_results = ggplot(metadata_model_test_adeq, aes(x = best_model, fill = best_model)) +
...
# Create the Boxplot
boxplot_model_steps = ggplot(metadata_model_test_adeq, aes(x = best_model, y = steps, fill = best_model)) +
  geom_boxplot() +
...
lmm_model_time <- lmer(interval_MY ~ best_model + (1| popID), data = metadata_model_test_adeq)
summary(lmm_model_time)

# Create the Boxplot
boxplot_model_time = ggplot(metadata_model_test_adeq, aes(x = best_model, y = interval_MY, fill = best_model)) +
  geom_boxplot() +
...
lmm_model_resolution <- lmer(resolution ~ best_model + (1| popID), data = metadata_model_test_adeq)
summary(lmm_model_resolution)

# Create the Boxplot
boxplot_model_resolution = ggplot(metadata_model_test_adeq, aes(x = best_model, y = resolution, fill = best_model)) +
  geom_boxplot() +
  labs(title = "Comparison of time series length (total time interval) with their best model",
       x = "Best model",
...
load("./results_paleoTS_v0.6.1/AICc_shift_results.RData")
