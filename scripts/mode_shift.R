...
#-------------------------------------------------
# FIT SINGLE MODE MODELS WITHOUT AND WITH A SHIFT
#-------------------------------------------------

### example of how to run model test done on HPC cluster 
#(takes time, load data used in article below) ###
...
## SUBSET 1
# test all possible single mode models with no shift from evoTS on every timeseries
#model_noshift_results_subset1 <- foreach(i = seq_along(ln_data_shift_subset1),
#                                         .packages = c("evoTS"),
#                                         .combine = "c") %dopar% {
#                                           nm <- names(ln_data_shift_subset1)[i]
#                                           res <- tryCatch(
#                                             fit.all.single_mode(ln_data_shift_subset1[[i]]),
#                                             error = function(e) {
...
## test all possible single mode models with shift from evoTS on every timeseries
...
#save(model_noshift_results_subset1, model_shift_results_subset1, 
#          file = "[PATH_TO_OUTPUT_FOLDER]/model_test_shift_subset1.RData")
#save(ln_data_shift_subset1, file = "[PATH_TO_OUTPUT_FOLDER]/ln_data_shift_subset1.RData")
...
#                                           
#                                           res <- tryCatch(
#                                             fit.all.single_mode(ln_data_shift_subset2[[i]]),
#                                             error = function(e) {
#                                               message(paste("Model", nm, "failed:", e$message))
#                                               NULL
#                                             }
...
#save(model_noshift_results_subset2, model_shift_results_subset2, 
#        file = "[PATH_TO_OUTPUT_FOLDER]/model_test_shift_subset2.RData")
#save(ln_data_shift_subset2, file = "[PATH_TO_OUTPUT_FOLDER]/ln_data_shift_subset2.RData")

## Loading datasets and results from the HPC
load("./data/ln_data_shift_subset1.RData") 
load("./data/ln_data_shift_subset2.RData")
load("./data/model_test_shift_subset1.RData") 
load("./data/model_test_shift_subset2.RData") 
...
### Remove problematic timeseries (or load data used in article below) ###
...
load("./data/ln_data_meta_shift.RData")
...
# write to file
#sink(file = "[PATH_TO_OUTPUT_FOLDER]/AICc_results_with_shift.txt")
...
#----------------------------------------------
# Time series described by more than one model 
# (AICcs with a difference inferior to 2 units)
#----------------------------------------------
...
#sink(file = "[PATH_TO_OUTPUT_FOLDER]/AICc_filter_with_shift.txt")
#paste("Total number of time series investigated:", length(aicc))
...
# (or load data used in article below)
...
#-------------------------------------------------------------
# Splitting the time series best described by mode-shift model
#-------------------------------------------------------------
...
# Saving OU subsets to work on them in another script (the adequacy tests of 
# OU models need to be implemented in a loop)
# see shift_OU_adequacy.R for the code
...
#     file = "[PATH_TO_OUTPUT_FOLDER]/OU_shift.RData")
...
Stasis_Stasis_subset1_adeq <- lapply(Stasis_Stasis_subset1, fit4adequacy.stasis, plot = FALSE)
Stasis_Stasis_subset2_adeq <- lapply(Stasis_Stasis_subset2, fit4adequacy.stasis, plot = FALSE)
names(Stasis_Stasis_subset1_adeq) = tsID_Stasis_Stasis
names(Stasis_Stasis_subset2_adeq) = tsID_Stasis_Stasis

Stasis_URW_subset1_adeq <- lapply(Stasis_URW_subset1, fit4adequacy.stasis, plot = FALSE)
Stasis_URW_subset2_adeq <- lapply(Stasis_URW_subset2, fit3adequacy.RW, plot = FALSE)
names(Stasis_URW_subset1_adeq) = tsID_Stasis_URW
names(Stasis_URW_subset2_adeq) = tsID_Stasis_URW

Stasis_GRW_subset1_adeq <- lapply(Stasis_GRW_subset1, fit4adequacy.stasis, plot = FALSE)
Stasis_GRW_subset2_adeq <- lapply(Stasis_GRW_subset2, fit3adequacy.trend, plot = FALSE)
names(Stasis_GRW_subset1_adeq) = tsID_Stasis_GRW
names(Stasis_GRW_subset2_adeq) = tsID_Stasis_GRW

Stasis_OU_subset1_adeq <- lapply(Stasis_OU_subset1, fit4adequacy.stasis, plot = FALSE)
names(Stasis_OU_subset1_adeq) = tsID_Stasis_OU

URW_URW_subset1_adeq <- lapply(URW_URW_subset1, fit3adequacy.RW, plot = FALSE)
URW_URW_subset2_adeq <- lapply(URW_URW_subset2, fit3adequacy.RW, plot = FALSE)
names(URW_URW_subset1_adeq) = tsID_URW_URW
names(URW_URW_subset2_adeq) = tsID_URW_URW

URW_GRW_subset1_adeq <- lapply(URW_GRW_subset1, fit3adequacy.RW, plot = FALSE)
URW_GRW_subset2_adeq <- lapply(URW_GRW_subset2, fit3adequacy.trend, plot = FALSE)
names(URW_GRW_subset1_adeq) = tsID_URW_GRW
names(URW_GRW_subset2_adeq) = tsID_URW_GRW

URW_OU_subset1_adeq <- lapply(URW_OU_subset1, fit3adequacy.RW, plot = FALSE)
names(URW_OU_subset1_adeq) = tsID_URW_OU

GRW_GRW_subset1_adeq <- lapply(GRW_GRW_subset1, fit3adequacy.trend, plot = FALSE)
GRW_GRW_subset2_adeq <- lapply(GRW_GRW_subset2, fit3adequacy.trend, plot = FALSE)
names(GRW_GRW_subset1_adeq) = tsID_GRW_GRW
names(GRW_GRW_subset2_adeq) = tsID_GRW_GRW

GRW_OU_subset1_adeq <- lapply(GRW_OU_subset1, fit3adequacy.trend, plot = FALSE)
names(GRW_OU_subset1_adeq) = tsID_GRW_OU

OU_GRW_subset2_adeq <- lapply(OU_GRW_subset2, fit3adequacy.trend, plot = FALSE)
names(OU_GRW_subset2_adeq) = tsID_OU_GRW

OU_URW_subset2_adeq <- lapply(OU_URW_subset2, fit3adequacy.RW, plot = FALSE)
names(OU_URW_subset2_adeq) = tsID_OU_URW

OU_Stasis_subset2_adeq <- lapply(OU_Stasis_subset2, fit4adequacy.stasis, plot = FALSE)
names(OU_Stasis_subset2_adeq) = tsID_OU_Stasis

GRW_URW_subset1_adeq <- lapply(GRW_URW_subset1, fit3adequacy.trend, plot = FALSE)
GRW_URW_subset2_adeq <- lapply(GRW_URW_subset2, fit3adequacy.RW, plot = FALSE)
names(GRW_URW_subset1_adeq) = tsID_GRW_URW
names(GRW_URW_subset2_adeq) = tsID_GRW_URW

GRW_Stasis_subset1_adeq <- lapply(GRW_Stasis_subset1, fit3adequacy.trend, plot = FALSE)
GRW_Stasis_subset2_adeq <- lapply(GRW_Stasis_subset2, fit4adequacy.stasis, plot = FALSE)
names(GRW_Stasis_subset1_adeq) = tsID_GRW_Stasis
names(GRW_Stasis_subset2_adeq) = tsID_GRW_Stasis

URW_Stasis_subset1_adeq <- lapply(URW_Stasis_subset1, fit3adequacy.RW, plot = FALSE)
URW_Stasis_subset2_adeq <- lapply(URW_Stasis_subset2, fit4adequacy.stasis, plot = FALSE)
...
#file = "[PATH_TO_OUTPUT_FOLDER]/adeq_shift_passed.Rdata")


#------------------------------------------------------------------------
# Table S2. Detail results of the models including mode-shift explaining 
# time series best according to relative and absolute fit.
#------------------------------------------------------------------------
...
# Relative percentage - percentage of time series that are fitted the best 
# by each model over the total number of time series before adequacy
...
# write to file
#sink(file = "[PATH_TO_OUPUT_FOLDER]/Results_fits_with_shift.txt")
#adeq_table
#paste("Total count    ", total_rc, "    ", total_c)
#paste("Time series not explained by stasis, unbiased random walk, or general random walk    ", 
#      round(total_notrad_rp,2), "    ", round(total_notrad_p,2))
#sink()



#------------------------------------------------------------------------------
# Table 3. Results of the types of models including mode-shift explaining time 
# series best according to relative and absolute fit.
#------------------------------------------------------------------------------
...
# write to file
#sink(file = "[PATH_TO_OUTPUT_FOLDER]/Results_fits_with_shift_summary.txt")
#adeq_table_summary
#paste("Total count    ", total_rc, "    ", total_c)
#sink()
